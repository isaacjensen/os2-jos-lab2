...
page_init(void)
{
	...
	size_t i;
	pages[0].pp_ref = 1; //physical page 0 in use
	...
	int EXTPHYSMEM_size = EXTPHYSMEM/PGSIZE;
	for(int i = PGNUM(IOPHYSMEM); i < PGNUM(npages_basemem * PGSIZE); i++) {
		pages[i].pp_ref = 1; //setting memory from [IOPHYSMEM, EXTPHYSMEM) to in-use 
		pages[i].pp_link = NULL;
	}
	
	for(int i = PGNUM(PADDR(boot_alloc(0))); i < npages; i++) { //iterate from first free to end
		pages[i].pp_ref = 0;
		pages[i].pp_link = page_free_list;
		page_free_list = &pages[i];
	}
...
page_alloc(int alloc_flags)
{
	...
	if(new_page) { //making sure we have a new page
...
page_free(struct PageInfo *pp)
{
	...
	//if(pp->pp_ref != 0 || pp->pp_link != NULL) panic("unable to free page"); //nonzero & non-NULL
	if(pp->pp_ref == 0) {
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if(pde & PTE_P) {
		//valid
		pte_t * page_table = (pte_t *)KADDR(PTE_ADDR(pde)); //get page table
...
{
	// Fill this function in
	pte_t *p_pte = pgdir_walk(pgdir, va, 1); //get page table entry of a va
	if(p_pte == NULL) return -E_NO_MEM; //error check
	pp->pp_ref += 1;
	if(*p_pte != 0) { //already mapped at va
